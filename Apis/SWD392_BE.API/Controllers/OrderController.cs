...
namespace SWD392_BE.API.Controllers
{
    ...
    public class OrderController : ControllerBase
    {
        ...
        public async Task<IActionResult> CreateOrder([FromBody] OrderRequestModel request)
        {
            ...
            var orderItems = request.FoodItems.Select(fi => (fi.FoodId, fi.Quantity, fi.Note)).ToList();

            var result = await _orderService.CreateOrderAsync(orderItems, request.VoucherCode);
